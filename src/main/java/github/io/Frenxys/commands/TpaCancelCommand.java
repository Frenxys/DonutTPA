...
public class TpaCancelCommand implements CommandExecutor {
    ...
    public TpaCancelCommand(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!this.plugin.getPermissionManager().hasPermission(player, "donuttpa.tpacancel")) {
            this.plugin.getMessageUtil().send(player, "no-permission", Map.of());
            return true;
        }
        this.plugin.getTpaManager().cancelRequests(player);
        return true;
    }
}
