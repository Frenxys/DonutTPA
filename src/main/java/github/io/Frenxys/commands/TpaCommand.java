...
public class TpaCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!this.plugin.getPermissionManager().hasPermission(player, "donuttpa.tpa")) {
            this.plugin.getMessageUtil().send(player, "no-permission", Map.of());
            return true;
        }
        if (args.length < 1) {
            this.plugin.getMessageUtil().send(player, "tpa-usage", Map.of());
            return true;
        }
        ...
        this.plugin.getTpaManager().initiateTPA(player, target, TPARequest.Type.TPA);
        return true;
    }
