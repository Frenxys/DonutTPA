...
public class TpaToggleCommand implements CommandExecutor {
    ...
    public TpaToggleCommand(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!this.plugin.getPermissionManager().hasPermission(player, "donuttpa.tpatoggle")) {
            this.plugin.getMessageUtil().send(player, "no-permission", Map.of());
            return true;
        }
        boolean on = this.plugin.getTpaManager().toggleTpa(player);
        this.plugin.getMessageUtil().send(player, on ? "tpa-toggle-on" : "tpa-toggle-off", Map.of());
        return true;
    }
}
